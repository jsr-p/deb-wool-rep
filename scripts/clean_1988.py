import csv
import pandas as pd
import numpy as np

year = '1988'

df = pd.read_parquet(f"data/brfss{year}.parquet")
df = df.rename(columns=dict(zip(df.columns, df.columns.str.lower())))
df.columns = ["x." + col[1:] if col.startswith("_") else col for col in df.columns]
income = df['income']
race = df['race']
state = df['x.state']
age = df['age']
sex = df['sex']
height = df['height']
weight = df['weight']

income_replace = {1:'<10k', 2:'10k-15k', 3:'15k-20k', 4:'20k-25k', 5:'25k-35k', 6:'35k-50k', 7:np.nan, 8:'>50k', 9:np.nan}
race_replace = {1:'white', 2:'black', 3:'hispanic', 4:'hispanic', 5:'hispanic', 6:'asian/pacific islander', 7:'native american', 8:'other/multiracial', 77:'refused/unknown', 99:'refused/unknown'}
age_replace = {7:np.nan, 9:np.nan}
sex_replace = {1:'male', 2:'female'}
hw_replace = {777:np.nan, 999:np.nan}
metric = False
max_weight = 777

income = income.replace(income_replace)
race = race.replace(race_replace)
age = age.replace(age_replace)
sex = sex.replace(sex_replace)
height = height.replace(hw_replace)
weight = weight.replace(hw_replace)

age = age.tolist()
new_age = []
for row in age:
    if row > 17 and row < 25:
        a = '18-24'
    elif row > 24 and row < 30:
        a = '25-29'
    elif row > 29 and row < 35:
        a = '30-34'
    elif row > 34 and row < 40:
        a = '35-39'
    elif row > 39 and row < 45:
        a = '40-44'
    elif row > 44 and row < 50:
        a = '45-49'
    elif row > 49 and row < 55:
        a = '50-54'
    elif row > 54 and row < 60:
        a = '55-59'
    elif row > 59 and row < 65:
        a = '60-64'
    elif row > 64 and row < 70:
        a = '65-69'
    elif row > 69 and row < 75:
        a = '70-74'
    elif row > 74 and row < 80:
        a = '75-79'
    elif row >79 and row < 100:
        a = '80+'
    else:
        a = np.nan

    new_age.append(a)

height = height.tolist()
new_height = []
for row in height:
    h = str(row)

    if row < 1:
        meters = np.nan

    elif row < 712:
        feet = float(h[0])
        inches = float(h[1:])
        if inches > 12:
            meters = np.nan
        else:
            inches = inches + feet*12
            meters = inches * 0.0254

    elif row < 9999 and row >= 9000 and metric:
        meters = float(h[1])+float(h[2:])*0.01
        if meters == 0:
            meters = np.nan

    else:
        meters = np.nan

    new_height.append(meters)

weight = weight.tolist()
new_weight = []
for row in weight:
    
    if row < 10:
        kg = np.nan

    elif row < max_weight:
        kg = row * 0.453592

    elif row < 9999 and metric:
        w = str(row)
        kg = float(w[1:])
        if kg < 10:
            kg = np.nan

    else:
        kg = np.nan

    new_weight.append(kg)

bmi = []
for h, w in zip(new_height, new_weight):
    b = w/(h*h)
    if b < 10 or b > 200:
        b = np.nan
    bmi.append(b)

age = pd.Series(new_age)
height = pd.Series(new_height)
weight = pd.Series(new_weight)
bmi = pd.Series(bmi)

mental_health = df['menthlth']
pm_health = df['poorhlth']
child1 = df['children']
child2 = df['x.chldcnt'] 
edu = df['educa'] 
brfss_out = pd.concat([ income, race, state, age, sex, height, 
weight, bmi, mental_health, child1, child2, edu, pm_health], axis=1)
brfss_out.columns = ['income', 'race', 'state', 'age', 'sex', 
'height', 'weight', 'bmi', 'mental_health', 'child1', 'child2', 
'edu', 'pm_health']
brfss_out.to_csv('clean/brfss'+year+'clean.csv')
print('Saved file to: clean/brfss'+year+'clean.csv')

